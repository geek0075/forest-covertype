...
The forest cover type project aims to build a machine learning model that predicts what types of trees grow in an area based on the surrounding characteristics using the [Forest Cover Type Dataset](https://archive.ics.uci.edu/ml/machine-learning-databases/covtype/covtype.data.gz). This dataset contains tree observations from four areas of the Roosevelt National Forest in Colorado. All observations are cartographic variables (no remote sensing) from 30 meter x 30 meter sections of forest. This dataset includes information on tree type, shadow coverage, distance to nearby landmarks (roads, water bodies, etc), soil type, and local topography.
...
Here is the accuracy of the best model when applied to the test set, which we have not used at all yet because the cross validation was done on the training set: 0.9649. Class specific summary statistics are shown in the table below.
...
Here is the accuracy of the best model when applied to the test set, which we have not used at all yet because the cross validation was done on the training set: 0.6788. Class specific summary statistics are shown in the table below.
...
Here is the accuracy of the best model when applied to the test set, which we have not used at all yet because the cross validation was done on the training set: 0.6982. Class specific summary statistics are shown in the table below.
...
But when we construct the ensemble from the top three individual models, *k-Nearest Neighbors*, *Random Forest* and *Support Vector Machines with Linear Kernel*, we obtain an overall ensemble accuracy of **0.9990087**, which is higher than the individual accuracies by using each model independently. This is shown in the table below.

```{r echo=FALSE}
ensemble_accuracies_top3 <- data.frame(
  kNN = c(
    0.9686
  ),
  randomForest = c(
...
kable(ensemble_accuracies_top3, caption = "Ensemble Accuracy vs. Component Model Accuracies: Top3")
```
...
From the accuracies presented in the results section, we conclude that if we were to pick just one model to predict forest cover type with it should be the *k-Nearest Neighbors* model. *k-Nearest Neighbors* takes the longest time to train and also takes the longest time to predict. It's delivered accuracies make up for this however. The next best performing model for predicting forest cover type is the *Random Forest* model, which also takes a long time to train, albeit not as long as *k-Nearest Neighbors* model. Predicting from the *Random Forest* model is somewhat as time consuming as it is with *k-Nearest Neighbors*.

#### Limitations

Infrastructure and time where a huge limitation during this analysis. Several of the machine learning algorithms trained on this project took days on end to train due to large dataset size. I think this will have been different if I had access to more infrastructure - faster hardware, more processing power, memory, and disk. This lack of infrastructure led me to reduce the time spent tuning model parameters.
